...
//     const { data: volunteerData } = await volunteersQuery;
...
//   return (
//     <div className="p-6 space-y-10">
//       <h2 className="text-3xl font-bold text-[#0057a8]">
//         Dashboard Overview
//       </h2>

//       {/* Stats */}
//       <div className="grid md:grid-cols-4 gap-6">
//         <StatCard title="Total Complaints" value={stats.complaints} />
//         <StatCard title="Pending Complaints" value={stats.pending} />
//         <StatCard title="Lost Items" value={stats.lost} />
//         <StatCard title="Volunteer Events" value={stats.volunteers} />
//       </div>
...
//   return (
//     <div className="bg-white shadow-lg p-5 rounded border-l-4 border-blue-500">
//       <h4 className="text-gray-500">{title}</h4>
//       <p className="text-2xl font-bold">{value}</p>
//     </div>
//   );
...
//               {editingId === item.id && type !== "complaints" ? (
//                 <>
//                   <input
//                     type="text"
//                     value={editTitle}
...
//                             : { event_name: editTitle, description: editDesc }),
//                         })
//                       }
...
//                     onClick={() => {
//                       setEditingId(item.id);
//                       setEditTitle(item.title || item.event_name);
//                       setEditDesc(item.description || item.message);
//                     }}
...
export default function Dashboard() {
  ...
  const updateStatus = async (table, id, value) => {
    await supabase
      .from(table)
      .update({ status: value })
      .eq("id", id);
...
function Section({ title, items, table, isAdmin, updateStatus, deleteItem }) {
  if (!items || items.length === 0)
    return (
      <div>
        <h3 className="text-xl font-semibold text-[#0057a8] mb-3">{title}</h3>
      ...
      <div className="space-y-3">
        {items.map(item => (
          <div
            key={item.id}
            className={`border p-4 rounded shadow flex justify-between items-center ${
              item.status === "resolved" ? "border-green-400" : "border-yellow-400"
            }`}
          >
            <div className="flex-1">
              <h4 className="font-semibold">
                {item.category || item.title || item.event_name}
              </h4>
              <p className="text-sm text-gray-700">
                {item.description || item.message}
              </p>
